...
\begin{figure}[!tb]
    ...
    \input{gfx/model/oct_tree.tikz}}}
    \subcaptionbox{collision 2d}[.65\textwidth]{
    \resizebox{!}{0.6\textwidth}{
    \input{gfx/model/collision_tree.tikz}}}
	\caption{}
	\label{fig:oct_tree}
\end{figure}
% 
...
% % 
% \ac{CC} are either renderd via the \textit{gluCylinder} function provided by the \textit{GLUT} library for a rough but fast visualiation.
...
% % 
% From the mesh, vertices and normals are calculated, which are then finaly rendered.
...
% % 
% \paragraph{Disclamer}
% This is a fast written approuch.
...
% % 
% A more advanced Tool, the \textit{FAConstructor} \cite{Reuter2019} was written by Andr'e Reuter in the context of this doctorial research.
This tool uses \textit{OpenGl 3} and additional calculation on the GPU.
...
% \section{Medusa}
% % 
% Additionally to the previous discussed Algorithm (\dummy), the \ac{MEDUSA} algorithm, was developed in a cooperation with the team Neurospin from \ac{CEA} in France \cite{Ginsburger2019}.
...
% % 
% Since this tool aims to build a statistically library, other parameters are chosen for boundry conditions.
These parameters are chosen as close as possible from the current \ac{dMRI} models:
% \dummy\\
% \dummy\\
% % 
% % Instead of choosing \ac{CC} the fibers and cells are represented as a collection of spheres.
...
% % 
% The calculation of collisions is done via the GPU architecture.
For this a first implementation was written with the \textit{AxisAligedSortedSearch} \cite{Karras2012}.
...
For this volume size the algorithm is for the current use fast enough.
...
% % \ac{dMRI} is here quite handy since it is currently the only technique to allow in-vivo measurements to analyse the orientation of white matter tracts.
...
% % %
% % \begin{quotation}
% % We design a novel white matter numerical phantom generation algorithm which constructs biomimicking geometric configurations with few design parameters, and enables to control the level of disorder of the generated phantoms.
The influence of various geometrical parameters present in white matter, such as global angular dispersion, tortuosity, presence of Ranvier nodes, beading, ...
% % \end{quotation}
% % %
