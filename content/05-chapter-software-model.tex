...
\begin{figure}[!t]
    ...
    \inputtikz[true]{gfx/model/min_torsion_wtf}}\hfill
    % 
...
% \afterpage{
% \begin{lstfloat}[!t]
% % \lstinputlisting[style=python,basicstyle=\footnotesize\ttfamily,]{code/collision_detection.py}
% \resizebox{\textwidth}{!}{
% \begin{sideways}
% \begin{tabular}{|cc|cc|}
% \hline
% \hspace{1em} &
% \begin{minipage}{0.4625\textheight}
% \lstinputlisting[style=cpp,basicstyle=\scriptsize\ttfamily,firstline=1,lastline=34]{code/collision_detection.py}
% \end{minipage} & \hspace{1em} &
% \begin{minipage}{0.4625\textheight}
% \lstinputlisting[style=cpp,basicstyle=\scriptsize\ttfamily,firstline=35,lastline=68,firstnumber=35]{code/collision_detection.py}
% \end{minipage} \\
% \hline
% \end{tabular}
% \end{sideways}}
...
\end{figure}
% 
\comment{A \say{tree} is a data structure consist of a collection of "nodes", which are connected to each other.
...
Therefore the first effort was to implement a structure which is reducing the number of collision checks between objects.
This can be done via pre sorting the objects into structures. 
There are again several existing methods for this.
The most important property for this kind of sorting is, that the objects have all the same order of magnitude in size.
This is the case, as we will see in \dummy.
\\[\baselineskip]
% 
The structure of an octree is the following (see \cref{fig:octreeCube}, \cref{alg:pseudocode_octree}).
% 
\begin{lstfloat}[!tb]
\lstset{style=python}
\begin{lstlisting}[]
...
\end{lstfloat}
% 
With this rather simple recursive algorithm the volume ill be split automatically into a \texttt{tree}.
% 
The careful part is the splitting process of the volume and objects.
When a volume is split into sub\_volumes a single object can be part of several (up to eight) sub\_volumes (see 2d example \cref{fig:octreeCube}).
Only if placed in such a way can the leafs be computed separately.
\footnote{This also means for a high overlapping coefficient a lot of branches can be filled with the same objects, hence the calculation time increases exponentially}.
The value of the threshold has to be manually set.
Usually the value is around \SI{20}{} objects.
It is a fight between the number of leafs (which grows $\mathcal{O}(\mathit{level})$ and the calculation inside a leaf.
The main goals is of course to have a minimal number of calculations.
However this depends heavily on the values of different parameters such as the \segLength or \segRadius (which will be explained later \dummy).
% 
\comment{
