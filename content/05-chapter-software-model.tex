...
\begin{figure}[!t]
	...
	\caption[]{Schematic of a fiber segment represented by a distance $\vec{d}$ and two radii $r_0$ and $r_1$ which describes a \acreset{CC} \ac{CC}.}
	\label{fig:conical}
\end{figure}
%
...
\end{lstfloat}
%
After a \code{step} the user is allowed to interact if necessary.
...
\par
%
An \name{octree} is a special \name{tree} where each node contains eight children, allowing a cubic volume to be divided into eight subvolumes.
...
%
%
\subsection{Optimization and parallelization}\label{sec:modelOpt}
%
