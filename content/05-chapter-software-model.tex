...
Therefore, one needs ideally a representation, which allows building any kind of geometry.
\par
%
...
If a collision occurs, the algorithm tries to remove it by moving the colliding parts slightly away from each other.
This step is repeated until the volume has no collisions.
\par
%
The nerve fibres simulation and all its necessary prerequisites is described in detail in this chapter.
...
\begin{figure}[!t]
	...
	\caption{Schematic of a fiber segment represented by a distance $\vec{d}$ and two radii $r_0$ and $r_1$ which descibes a \acreset{CC} \ac{CC}.}
	\label{fig:conical}
\end{figure}
%
...
\begin{tikzpicture}[remember picture, overlay]
    \begin{scope}[shift={(current page.center)}]
        \node[align=center,white](0,0) {\Huge \bfseries So Long, and Thanks for All the Fish};
    \end{scope}
\end{tikzpicture}
%
%
...
From there, the fiber is circularly bent until the second direction angle $\beta$.
...
If any point $\vec{p}_{i}$ in a fiber fails these boundary conditions, the three points will be shifted $\vec{p}_{i-1}, \vec{p}_{i}$ and $\vec{p}_{i+1}$ will be moved parallel to reduce curvature (see \cref{fig:modelCircle}).
% 
\begin{figure}[!t]
    \centering
...
% 
% 
\section{Sphered Nerve and Cell Modeling}
\label{sec:medusa}
%
