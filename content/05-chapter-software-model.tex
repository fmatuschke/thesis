...
\begin{align}
\mathit{seeds} = \left\{ \vec{p}_i=(x_i,y_i,z_i) \mid x,y,z \in \mathbb{R} , \, i \in \{0,1,N_{\mathit{seed\_points}}-1\}\right\}
\end{align}
% 
...
\end{align}
% 
where $\vec{f}(s)$ is a parameterized trajectory of the parametric variable $s$, $\vec{t}(s)$ is the tangential vector, $\vec{n}(s)$ is the principal normal vector and $\vec{b}(s)$ is the binormal vector (see \cref{fig:torsionCurve}).
One could use this idea to rotate the seed points into the coordinate system of $\vec{t}(s),\vec{n}(s),\vec{b}(s)$.
However, this can lead to an arbitrary rotation of the fibers, \eg{} if the curve is described in such a way that $\vec{n}(s),\vec{b}(s)$ would continuously rotate around $\vec{t}(s)$.
\\
% 
...
When defining a plane, within the vectors $\vec{n}(s),\vec{b}(s)$ defined above, the plane is rotated from one step $i$ to the next step $i+1$ by the same amount by which the curve bends.
However, the rotation along the vector $\vec{t}(s)$ should be $0$.
Since a discrete point exists, the plane can be rotated by the same amount as the vector $\vec{t}(s)$.
However, to obtain a plane in the center of the points $\vec{p}_{i-1}, \vec{p}_{i}, \vec{p}_{i}, \vec{p}_{i+1}$ the middle orientation of these points will be used.
...
This method allows to create fibers within the cuboid volume that are oriented along a user-defined main orientation $\vec{v}$ (see \cref{fig:cubeBuild}). 
The individual fibers are also initialized with seed points.
An infinite line with the main orientation $\vec{v}$ is laid through each seed point.
