...
However, it has already been shown that irregular, non-symmetric nerve fiber configurations are necessary to obtain a realistic result without diffraction patterns \cite{MenzelDissertation}.
...
\caption{Collision detection between two capsule objects. The distance as well as the points on the line segments is returned. A collision takes place if the distance is smaller than $\mathit{cone_a.r}+\mathit{cone_b.r} > d$. \todo{same variable name as in figure and text}}
...
\begin{figure}[!t]
    ...
    \subcaptiontab{0.475\textwidth}{Boundary segment length: lower bound $\varphi=\SI{60}{\degree} \xrightarrow{} r_{\mathit{min}} \geq \SI{0.5773}{} \cdot l_{\mathit{mean}}$} &
    \subcaptiontab{0.475\textwidth}{\label{fig:boundaryLength}Boundary segment radii: lower bound $r_{\mathit{min}} \geq \fiberRadiusMean$}
    \end{tabular}
...
$p_{i-1},p_{i+1}$ is moved along the shortest distance vector and $p_{i}$ in the opposite orientation.
...
\label{sec:medusa}
%
The above described algorithm is capable of generating dense \ac{WM} nerve fiber models.
In \ac{dMRI} these models can be applied as well.
...
This, however, is not only true for nerve fibers but also for ather types of cells.
Since the \ac{WM} consist not only out of axons, but also out of gliacells like astrocytes and olegodendrites, these are also infering with the movement of the water moelcules.
\\
% 
...
This has the advantage, that the objects are simpler, which leads to an increase in collision checking calculations, but also spheres are capable of approximating a 3d object like a cell body.
Latter is achieved by representing the surface of a cell body as the sum of multiple spheres. 
Only the surface of a sphere, which is not colliding with another sphere is \dummy{} as the surface of the entire cell (see \cref{ig:medusaCell}).
...
Nerve fibers are initialized as fiber populations with parameters like density and angular dispersion (see \cite[Ginsburger2019]).
...
\begin{figure}[!t]
    ...
    \\[2em]
    % 
    \subcaptionbox{Cell aproximation by overlapping spheres.}[0.75\textwidth]{\tikzset{external/remake next}
