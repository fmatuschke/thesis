...
% \\
% radius
The results indicate, that with expection for mean fiber radii $\geq \SI{5}{\micro\meter}$ the volume fraction does not change its behavier with a change of the mean fiber radii.
For values $\geq \SI{5}{\micro\meter}$ the variance increases significantly.
Nevertheless, the median, even if it becomes smaller, follows the same behavior as for the smaller fiber radii across all parameters.
\par
% 
% singel fiber
Single fiber populations $(||)$ are very high with values $> \SI{0.75}{}$.
There value shows no significant change over the fiber bending radius factor \segRadiusFactor{}.
There is only a slightly decrease visible for increasing fiber segment length factor \segLengthFactor{}.
\par
% 
% crossing fiber
This behavier changes significantly for the crossing fiber population $(\times)$.
There values are always smaller than for the single fiber population case.
For $\segLengthFactor{}=\SI{1}{}$ the volume fraction decreases abrupt between $\segRadiusFactor{} = \SI{2}{}$ and $\segRadiusFactor{} = \SI{4}{}$. 
The change becomes smoother for $\segLengthFactor{}=\SI{2}{}$.
The overall volume fraction decreases for values $\segLengthFactor{} \geq \SI{4}{}$ compared to the previous values.
For $\segLengthFactor{}=\SI{8}{}$ the change with \segRadiusFactor{} disapears and the volume fraction only reaches a value around $\S{0.55}{}$.
% 
% 
% 
\par
