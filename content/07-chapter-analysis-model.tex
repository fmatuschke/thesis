...
AT THE END OF EACH STEP THE VOLUME TO SOLVE IS ALWAYS CUTTED TO 105.
% 
\begin{table}[!b]
...
\caption[Time development parallel]{Time development of the model generation process of parallel fiber populations. crossing \itodo{\segLengthFactor and \segRadiusFactor}\itodo{title}\itodo{units}}
...
\\
% 
The behavier in change of $segLengthFactor$ is clearly a time advantage, since also the number ob objects is smaller.
In a log plot the change is not clearly visible, since the reduction is only linear.
However the number of collsions is clearly 
% 
\vspace{5pt}
\hrule
\vspace{6pt}
