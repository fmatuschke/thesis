...
fiber bundle crossing/proportion & $\modelPsi$/$\modelOmega$ & $\SI{1.0}{}/\SI{0}{\degree}$, $0.5/\SI{90}{\degree}$\\
}
...
\begin{figure}[p]
\centering
\includegraphics[page=1]{dev/rc1/pre_stats_box_plot_volume_output_parameter_statistic_rc1.pdf}
...
\includegraphics[page=2]{dev/rc1/pre_stats_time_evolve_output_parameter_statistic_rc1.pdf}
...
\includegraphics[page=1]{dev/rc1/pre_stats_time_evolve_output_parameter_statistic_rc1.pdf}
...
% 
% SINGLE
The single fiber populations show a strong linear correlation between the computation time and the number of steps for all parameters.
The number of steps increase significantly with decreasing \segLengthFactor{} as well as decreasing \fiberRadiusMean{}.
The increase in computational time is about one magnitude with each decreasing step in \fiberRadiusMean{}.
...
\\
% 
The overlap fraction show a behavior with increasing computational time.
...
At the end of every parameter set the variance of the overlap fraction increases significantly.
\\
% 
The number of objects show only a slightly decrease over time, which is almost not visible in the logarithmic plot. 
As before \segRadiusFactor{} show no effect in the number of fiber segment.
A significant increase is visible in a decreasing value of \fiberRadiusMean{} and \segLengthFactor{}.
\par
% CROSSING
% 
The crossing fiber population in \cref{fig:timeDevelopmentCross} shows a linear behavior for the number of steps as well for all parameter sets.
...
The exponential decrease seams to split at a critical number of steps (or computational time) and only decreases linear from there on for higher \segRadiusFactor{}.
This splitting however only occurs for the smaller \segLengthFactor{}.
The slop of the second phase is decreased for higher \segRadiusFactor{}.
This behavior is more and more evident with decreasing \fiberRadiusMean{} and \segLengthFactor{}.
The splitting point with regards to the computational time very close to the total computational time of the lowest \segRadiusFactor{}.
\\
% 
The overlap fraction shows for the smaller fiber segment length factor  \segLengthFactor{} a different with respect to the single fiber population case.
For the smaller \segLengthFactor{} the curve almost follows a logistic curve.
Where for the higher \segLengthFactor{} the curve stops at the turning point, for the smaller values it continues and the number of overlap fraction less and less decreases until it almost stays constant until the model is solved or the total number of steps is reached.
Again the mean fiber radius \fiberRadiusMean{} does not change the curve significantly.
The lower values for the overlap fraction is as well about $\SI{1}{\percent}$ at the end of the solving steps.
\\
% 
The number of objects show the same behavior as in the single fiber population case. 
A slightly decreasing of the number and a significant increase is visible in a decreasing value of \fiberRadiusMean{} and \segLengthFactor{}.
\par
% 
All missing parameters from the parameter series are listed in \dummy{}.\todo{appendix}
% 
% 
% 
...
% 
% 
\section{Repo Messungen und weitere Laufzeit analysen}
% 
\begin{table}[!b]
\resizebox{\textwidth}{!}{
\pgfplotstabletypeset[%
