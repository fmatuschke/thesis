...
The first part focuses on the determination of all necessary physical parameters of the tissues and microscope as well as the characterization of the simulation parameters.
The latter is focused on the \voxelsize{}, which significantly determines the accuracy.
Then, these parameters are used to simulate the previously generated models.
The analysis of the simulations is done with the routine algorithms implemented in \ac{fastPLI}. The focus of the evaluations of the results is on the accuracy of the slope analyses for different orientations and intersection configurations.
% 
% 
% 
...
\end{figure}
%
To measure the optical resolution of the microscope, the measurements and analysis analog to \cite{MenzelMaster} \todo{check ref} is done again.
For this the \textit{1951 \ac{USAF} resolution test chart}\footnote{U.S. Air Force MIL-STD-150A standard of 1951} is acquired.
...
To identify the line width, the patches are numerical ordered by a main group $i$, and a subgroup $j$.
\par
% 
Resolution is measured by measuring a microscopic image of the \ac{USAF} test chart.
To determine this, the group corresponding to the imaging resolution system must be identified.
This is done by analyzing the intensity profiles along the perpendicular line of the three slits. 
The variance of the measured intensity is measured by analyzing all parallel lines in the slits.\footnote{Normally, multiple images should be measured. However, this is already done by \cite{MenzelMaster}. This measurement ensures that the results are reproducible}.
% 
\paragraph{Results}
...
\leavevmode\subcaption{\label{fig:parameterModelNoise} Noise analysis PM. $\opticgain_{\mathit{PM}} = \SI{0.1175}{}(?)$.}
\end{minipage}}
&
\multicolumn{1}{l}{
\begin{minipage}[t]{0.495\textwidth}
...
% 
\paragraph{results}
The results are shown in \cref{fig:parameterModelNoise} and show a gain value of $\opticgain_{\mathit{PM}} = \SI{0.1175}{}(?)$ which is agreement to the hardware specifics \todo{check and ref}.
This gain factor can be used by the model:
\begin{align}
f(x) = \mathrm{round}\left(\mathrm{normal}(\mu = x, \sigma=\sqrt{\opticgain \cdot x})\right)
\end{align}
\todo{only values > 0 and I >> sqrt(gI)}
% 
% 
% 
...
% \tikzset{external/export next=false}
\subcaptionbox{zoom transmittance}{\inputtikz{gfx/data/vervet_transmittance_zoom}}\hfill
% \tikzset{external/export next=false}
\subcaptionbox{hist transmittance}{\inputtikz{gfx/data/vervet_transmittance_hist}}
\hspace*{\fill}
\\
\hspace*{\fill}
% \tikzset{external/export next=false}
\subcaptionbox{zoom retardation}{\inputtikz{gfx/data/vervet_retardation_zoom}}\hfill
% \tikzset{external/export next=false}
\subcaptionbox{hist retardation}{\inputtikz{gfx/data/vervet_retardation_hist}}
\hspace*{\fill}
...
\includegraphics[width=0.95\textwidth]{dev/rc1/tissue/bf_rc1_transmittance_PM_Vervet.pdf}} %\hfill
...
\caption{simulations for Vervet PM tissue for different absorption coef and birefringence values. \itodo{exp messungen mit einzeichnen, transmittance nur ein wert}}
...
Since the focus will be the Vervet monkey\todo{argue why} (since a large range of registered data is available at this moment), a coronal section will be used to analys both properties.
...
% 
\Cref{fig:brain_ret_trans} shows the transmittance and retardation maps of the coronal Vervet section, and \ref{fig:brain_ret_trans_zoom} a zommed version.
...
However, since this property is mathematically the same, namely a reduction in intensity along the tissue, \fixme{it can still be used}, but it must be remembered that it is the \fixme{outdated} "absorption" and not just the loss of energy in the tissue.
...
\fixme{However, it should be a good estimate} for the magnitude of absorption in the tissue, which, as will be shown later in the simulation, is sufficient.
% 
\itodo{Messung beschreiben}
% 
\paragraph{results}
...
% 
% 
\section{Parameterization/ Model library}
% 
% \begin{figure}[p]
% \centering
% \setlength{\tikzwidth}{0.45\textwidth}
...
% \end{figure}
% 
Def of angles, def of parameters ...
% 
% 
% 
\section{Repo}
...
\end{axis}
\end{tikzpicture}
\caption[sim]{rofl dir mean  \dummy{}}
% \label{fig:sim_fyjsrg}
\end{figure}
% 
\begin{figure}[!p]
...
\end{axis}
\end{tikzpicture}
\caption[sim]{rofl trel mean 0-0.55 \dummy{}}
% \label{fig:sim_fyjsrg}
\end{figure}
% 
