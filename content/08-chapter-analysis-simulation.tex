...
\begin{figure}[!t]
\centering
\includegraphics[width=0.75\textwidth]{dev/gfx/sim_cube/sim_acc_gt_r.pdf}
% \includegraphics[width=0.75\textwidth]{dev/gfx/sim_cube/sim_acc_gt_p.pdf}
\caption[sim acc]{sim\_acc \dummy{}}
\label{fig:parameterModelSimGain}
\end{figure}
% 
...
\item anfang
    \begin{enumerate}
    \item Welchen einflu\ss hat mu? theoretisch abhandelbar
...
\item mitte
    ...
    \item Rofl abweichend bei starker Retardierung?
    \end{enumerate}
% 
\item ende?
    \begin{enumerate}
...
\begin{figure}[!tp]
\centering
\includegraphics[width=\textwidth]{dev/gfx/sim_cube/simulation_analysis_hist_0.5_PM_Vervet_r_acc_False.pdf}
% 
\begin{tikzpicture}
 \begin{axis}[
...
% \begin{align}
% R = \frac{\sum_i f(\theta_i, \varphi_i) -  f(\theta_{\mathit{rofl},i}, \varphi_{\mathit{rofl},i})}{\sum_i f_0(\theta_i, \varphi_i) -  f_0(\theta_{\mathit{rofl}sd,i}, \varphi_{\mathit{rofl},i})}
% \end{align}
% % 
% The highest accuracy for all models is at the $\omega=\SI{0}{\degree}$ position.
% The overall accuracy is higher for $\Psi = 0.3$.
% For higher $\Psi$ values the accuracy seems to localise around the f0 vector \todo{WHY?} \TODO{MAKES NO SENSE?}.
