...
\ac{HDF5} allows to store the data as \code{datasets} into \code{groups}.
This is analog to a file in a operating systems, which is stored into folders.
The \ac{HDF5} groups are used to store the \code{fiber} into \code{fiber\_bundle} and \code{fiber\_bundles}.
...
It is also possible to save the class state, with the current state of the \code{fiber\_bundles}, as a \ac{HDF5} object.
...
Methods to save the variables as \code{dict} or \ac{HDF5} files are available as well.
\par
