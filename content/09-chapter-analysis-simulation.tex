...
Then the model library is simulated with all sampled orientations.
The orientation of the simulations is calculated using the routine algorithms implemented in \ac{fastPLI}.
The focus of the evaluation of the results is on the accuracy of the tilt analysis for different orientations and crossing configurations.
%
%
%
...
\caption{%
\dummy{}
% Transmittance left: $1200 \pm 500$,
% right: $1200 \pm 500$.
% Background transmittance: $4530 \pm 240$.
% fuellgrad $0.75: \mu \approx 30 \pm 10$,
% fuellgrad $0.75: \mu \approx 30 \pm 10$,
% Retardation left: $0.83 \pm 0.07$,
% right: $0.80 \pm 0.07$.}
}
...
% % \centering
% \begin{tabular}{cc}
% \includegraphics[page=2]{gfx/rc1/tissue/birefringence_output_bf_rc1.pdf}&
% \includegraphics[page=5]{gfx/rc1/tissue/birefringence_output_bf_rc1.pdf}\\[1em]
% \multicolumn{2}{c}{\includegraphics[page=7]{gfx/rc1/tissue/birefringence_output_bf_rc1.pdf}}
% \end{tabular}
% \caption{Simulation results for transmittance (left) and birefringence (right) for different fiber radii $\fiberRadiusMean$ of a single flat fiber population.
% The transmittance variance increases significantly with increasing fiber radius.
% For larger fiber radii the mean value can change.
% The retardation mean value decreses for increasing fiber radii while the variance increases.
% Larger birefingence strength result in a larger retardation.
% The experimentally measured retardation of about $\SI{80}{\percent}$ (see \cref{fig:brain_ret_trans_zoom}) is reached with a birefringence strength of about $\dn \approx \si{0.008}{}$.
% }
...
To measure the values it is important that a homogeneous region is selected.
...
\par
% 
\Cref{fig:brain_ret_trans} shows transmission and retardation maps of the of the vervet monkey coronal section number $\SI{549}{}$.
...
The same measurements are made for human and rodent in the corpus callosum (see Appendix \todo{label}).
\par
% 
...
%
Optical resolution depends amongs other effects on aberration and diffraction (see \cref{sec:opticalResolution}).
They will be moddeld as described in \cref{sec:ccdOptic}.
Therefore the models parameters have to beevaluated.
\par
%
...
For this purpose, the intensity profiles perpendicular to the the three slits (vertical and horizontal) are analyzed.
...
%
\subcaptiontab{0.475\textwidth}{\label{fig:usaf_lines_lr}Centered line plots left to right. \todo{axis}} &
\subcaptiontab{0.475\textwidth}{\label{fig:usaf_lines_ud}Centered line plots top to bottom. \todo{axis}}
% 
\end{tabular}
...
This requires several captured images with different pixel intensities.
...
\par
%
To investigate this effect, a simulation is performed the voxel sizes of the range $\voxelsize = \SIrange{0.01}{1.3}{\micro\meter}$.
The smallest voxel size $\SI{0.01}{\micro\meter}$ is used as reference.
Since this voxel size is so small, only a volume of $3 \cdot \SI{1.3}{\micro\meter} \cdot \SI{1.3}{\micro\meter} 3 \cdot \SI{1.3}{\micro\meter} \times \SI{60}{\micro\meter}$ is used without light beam tilts.
...
\end{align}
and therefore aquidistant inside the $xy$-plane of the model.
% 
% 
% 
\paragraph{Results and discussion}
...
The models are tilted in steps of $\Delta \modelInc = \SI{5}{\degree}$ for a single fiber population and by $\Delta \modelInc = \SI{30}{\degree}$ for crossing fiber populations. The latter are then also rotated around the first fiber population (see \cref{fig:simModelSphereSample}).
...
The number of samples is an integer close so that the distance between two inclinations are \dummy{} over the sphere.
\todo{improve view angle}}
[.475\textwidth]{\inputtikz{gfx/model/sphere_models_c}}
\hfill
\subcaptionbox{The results of a value over all orientations of the second fiber population $\popb$ is represented as polar plot.
The orientation of the first fiber population $\popa$ is marked as a black circle.
The \dummy{} let mirror the results at the x-axis.}
...
\end{figure}
% 
% \subsection{Transmittance and retardation}
% % 
% The evalueted absorption and birefringence are used to simulate the variance of the modelled tissue with unterschiedlichen fiber radii.
% % 
% \begin{figure}[!t]
% % \centering
% \begin{tabular}{cc}
...
\includegraphics[page=2]{gfx/rc1/speed/boxplot_generation_output_generation_mpi_v_0.1.csv.pdf}
\end{minipage}
&
...
\begin{tabular}{cc}
\begin{minipage}{0.6\textwidth}
\includegraphics[page=2]{gfx/rc1/speed/boxplot_simulation_output_simulation_mpi_v_0.1.csv.pdf}
\end{minipage}
&
\begin{minipage}{0.25\textwidth}
\usebox{\newtable}
...
\begin{tabular}{cc}
\begin{minipage}{0.6\textwidth}
\includegraphics[page=2]{gfx/rc1/speed/boxplot_generation_output_generation_mp_v_0.1.csv.pdf}
\end{minipage}
&
\begin{minipage}{0.25\textwidth}
\usebox{\newtable}
...
\begin{tabular}{cc}
\begin{minipage}{0.6\textwidth}
\includegraphics[page=2]{gfx/rc1/speed/boxplot_simulation_output_simulation_mp_v_0.1.csv.pdf}
\end{minipage}
&
\begin{minipage}{0.25\textwidth}
\usebox{\newtable}
