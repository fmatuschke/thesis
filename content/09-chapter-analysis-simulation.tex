...
The entire section and the analysis for a human and a rodent section are described in the Appendix (see \cref{app:brain_rodent,app:brain_vervet,app:brain_human}).
% 
\begin{figure}[!t]
...
The models to be simulated are parallel models without inclination $\pfbs, \modelInc = \SI{0}{\degree}$, inclined parallel fiber models $\pfbs, \modelInc = \SI{90}{\degree}$, crossing flat fiber models $\cfbs, \modelInc = \SI{0}{\degree}$, and crossing inclined fiber modls $\cfbs, \modelInc = \SI{90}{\degree}$ with a fraction of $\modelPsi = \SI{0.5}{}$, since these configurations represent the extrema for two fiber populations.
For statistic the simulations are repeated $\SI{25}{}$ times at the $x$ and $y$ positions:
\begin{align}
% \begin{split}
...
\par
% 
Three types of simulations are performed.
First, an inclined fiber population.
...
% 
% 
\subsection{Inclined fiber population}
...
\begin{figure}[!t]
    ...
    \caption{Inclined fiber population orientation histograms. \todo{alpha, bzw theta etwas größer?}}
...
\par
% 
\Cref{fig:single_fiber_pop_rofl} shows the results of the simulation on the inclined fiber models.
\par
% 
\begin{figure}[!p]
    \centering
    \includegraphics[]{gfx/rc1/analysis/plots_single_pop_output_cube_2pop_135_rc1_single.pdf}
    \caption{Inclined fiber population. Results of the simulation analysis with comparison of the fiber models orientations.}
...
\section{Discussion}
%
\paragraph{Inclined fiber population}
% 
\todo{inclined? single? also check images}
\par
% 
...
\section{todos}
\begin{itemize}
    \item check "in summary" for all parts
    \item check titles of images
    \item check if all images are referenced
\end{itemize} 