...
%
The circmean direction results are shown in \cref{fig:sim_ana_dir} and the circmean of the inclination in \cref{fig:sim_ana_incl}.
The interpolation method is not used for the angular values since on a half sphere manifold the needed interpolation are not straight forward.
The above used k-nearest neighbor interpolator would lead to false results.
% 
\begin{figure}[!p]
    \centering
...
For steep fibers with a $\modelInc = \SI{90}{\degree}$ the second fiber population orientation is the result regarding on the fiber population fraction \modelPsi{}.
For fiber population fractions close to $\SI{50}{\percent}$ the resulting orientations behave acording the average of the two individual orientations with respect to the fiber population fraction.
% 
% 
% 
