...
%
Optical resolution is dependent on aberration and diffraction, which are in turn dependent on a multitude of parameters (see \cref{sec:opticalResolution}).
They will be moddeld as described in \cref{sec:ccdOptic}.
First, however, the necessary parameters for the models have to be evaluated.
\par
%
\begin{figure}[!t]
\centering
...
\end{align}
for intensities $I > 0$, $I \gg \sqrt{\opticgain I}$.
%
%
%
...
\end{tabular}
\caption{Resulting transmittance and birefringence strengths at different \Voxelsize s \voxelsize{} for flat fiber population simulations.
The transmittance depends only on the voxelsize and its variance increases with increasing voxelsize.
The experimentally measured retardation of about $\SI{80}{\percent}$ (see \cref{fig:brain_ret_trans_zoom}) is reached in the simulation with a birefringence strength of about $\si{0.008}{}$. Larger voxelsizes result in a higher variance.}
...
Since the models created here use stiff fiber segments, the volume density cannot reach the high density normally present in \ac{WM}.
...
Since the noise in this microscopic configuration is quite low, it is unlikely to affect the results.
\par
%
...
The absorption for rodent is about $\SI{8}{\milli-meter-tothe{-1}}$ and human is about $\SI{65}{\milli-meter-tothe{-1}}$.
\par
%
