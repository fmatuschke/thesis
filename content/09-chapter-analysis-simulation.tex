...
\Cref{fig:flat_03_fiber_pop_hist,fig:flat_05_fiber_pop_hist} shows the distribution of orientations for the model fiber segments and the resulting simulation orientations for $\modelPsi = \SI{30}{\percent}$ and $\modelPsi = \SI{50}{\percent}$.
...
\par
% Direction
The direction for all simulated fiber population fractions \modelPsi{} follows the theoretical \name{circmean} function.
...
Depending on the volume fraction the variance increases with the crossing angle, for fiber population fractions close to $\SI{0}{\percent}$ and $\SI{100}{\percent}$ or remains constant for 
...
The additional fiber population fraction are listed in \cref{app:incl_fiber_pop_a,app:incl_fiber_pop_b,app:incl_fiber_pop_c,app:incl_fiber_pop_d,app:incl_fiber_pop_e}.
\par
% Transmittance
The transmittance increases with increasing crossing angle \modelOmega{}.
...
No significant influence is visible for the fiber population fraction with the exception of $\modelPsi = \SI{10}{\percent}$ where for a crossing angle $\modelOmega >= \SI{80}{\degree}$ the variance increases significantly.
The direction variance of the simulation is significant lower than the nerve fiber models segements variance.
...
Depending on the fiber population fraction \modelPsi{} the circmean between both fiber population is followed by the simulation results for lower crossing angles.
Higher crossing angles $\approx \geq \SI{50}{\degree}$ are significantly lower then the circ mean value.
