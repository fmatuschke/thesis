...
For $\modelInc=\SIlist{85;90}{\degrees}$, the variance then increases significantly and for $\modelInc=\SI{90}{\degrees}$, the mean is $>\SI{1}{}$.
This indicates that the tilt analysis is unable to find a local minima for \trel{} values $<1$.
This may be due to several reasons.
First, there is no local minima with $\trel<1$.
Second, there is a local minima, but the optimizer is not able to find it with the initial values.
Finally, there is a local minima, but the solution found with a \trel{} value $>1$ is a better choice, \ie{} has a lower $R^2$ value.
...
For the case $\modelPsi=\SI{50}{\percent}$ and $\modelOmega=\SI{90}{\degree}$, the direction values are uniformly distributed since there is no unique direction in the sinusoid.
This is the effect of delay canalatation and signal noise.
\par
%
...
Because of the lower volume fraction, the \trel{} value must be lower.
\par
%
...
%
This section shows the results of speedup measurements for discretized tissue volume generation (see \cref{sec:dv_generator}) and light matter interaction simulation (see \cref{sec:simulation}) for the implemented paralellizations with \ac{OpenMP} and \ac{MPI}.
...
\par
% 
The speedup for the \ac{MPI} implementation, on the other hand, is almost opitimal up to $\SI{8}{\cores}$.
