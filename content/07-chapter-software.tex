...
In addition, the algorithms should be easy installable to provide easy access.
\par
%
...
The wiki page is structured as a guide that walks through the aspects of designing nerve fiber models, applying the collision solution algorithm, visualizing nerve fibers, a introduction in \ac{3D-PLI}, and finally applying the models in the simulation.
...
%
Each module with its main methods is automatically tested with a \name{Github action} \footnote{github actions are commonly used to automatically build, test, and deploy the software and documentation}. after each \name{git push}.
...
Each attribute checks if the user input valid and returns an appropriate warning or error message if necesarry.
...
Finally, this class also provides the possibility to use a simple visualization of the solution process (see \cref{sec:visualization}).
%
%
%
...
\par
% 
Usually as in the experiment the flat and four tilt measurments are simulated.
