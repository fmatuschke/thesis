...
The minimization of the $xy$ surface area is important for the simulation algorithm.
\\
...
However since the light beam also needs in the case of interpolation the information of the sorounding 8 voxels, this would mean that the missing voxels had to be tranfered to the neighboring ranks.
% 
\begin{figure}[!t]
\centering
\def\tikzwidth{0.75\textwidth}
...
\end{figure}
% 
This would mean a huge amount of communications, which are relativly slow.
The solution is a so called \name{halo}.
This is a commenly used concept, where the borders (in this case a volume) are enlarged by a a specific size (in this case 1 voxel) so that the same information of the shared reagions is available everywhere (see \cref{fig:com_halo}).
% 
\begin{figure}[!t]
\centering
\includegraphics[width=0.5\textwidth]{example-image-a}
\caption{light send to another node}
\label{fig:com_halo_light}
\end{figure}
Now the light beam can be communicated when it leaves the local volume to the next one (see \cref{fig:com_halo_light}).
