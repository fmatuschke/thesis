...
% 
\dummy[wohin mit den zukunftsplaenen?]{}.
% 
% 
...
\textcolor{gray}{
\begin{itemize}
    \item how to represent a fiber segment in a disc volume
    \item layer
    \item optical axis
    \item loop over voxels
    \item loop over all segemnts
    \item parallelization
\end{itemize}
}
% 
the question how to discretize the previous defined nerve fiber models (see \cref{chap:sof:modelling}) can be simplified in how to discretize a single nerve fiber segment, since a fiber is a chain of consecutive segment.
The idea is, that if a voxel is inside a fiber segment, it will be marked as a tissue with the physical properties (absorption, optical axis and birefringece) at the voxels center position.
Here a voxel inside a fiber segment means, that its center point is inside the volume of the fiber segment.
A voxel at the array position $[i,j,k]$ takes up the space of $(i,j,k)$ to $(i+1,j+1,k+1)$ in the unit of \voxelsize{}.
The overall idea can be represented in the pseudocode \cref{alg:fiberbundleprops}.
% 
\begin{lstfloat}[!tb]
\lstset{style=python}
\begin{lstlisting}[]
for i,j,k in voxels:
    ...
    if min_dist < cc.radius:
        if min_dist < current_distance[i,j,k]:
            optical_axis[i,j,k,:] = get_axis_orientation((i,j,k), min_dist, min_point)
            tissue[i,j,k] = get_layer_id(min_dist)
            current_distance[i,j,k] = min_dist
\end{lstlisting}
\caption{single \ac{CC} checking algorithm.}
\label{alg:fiberbundleprops}
\end{lstfloat}
% 
\par
% 
% Two coordinate systems exists.
% The first is the array indices $(i,j,k)$ which corresponds to the $\mathit{array}[i,j,k]$ position inside the array.
% % 
% \begin{figure}[!t]
% \centering
...
% \end{figure}
% % 
% The second is the coordinate system is in the unit of \code{voxel\tu sizes}.
% This means, that a voxel $(i,j,k)$ takes up the space of $(i,j,k)$ to $(i+1,j+1,k+1)$ in the second coordinate system (see \cref{fig:indexing_array}).
% The second coordinate system multiplied by the \code{voxel\tu size} \voxels{} corresponds to the world in \si{\micro\meter}, and therefore the same as the fiber models.
% The transformation is a simple linear function $p=(i+0.5,j+0.5,k+0.5) \cdot \voxels$.
% % 
...
\par
% 
The next step is to fill the voxels of the volume with the fiber segment.
A nerve fiber segment is a \ac{CC}.
This means, that at each voxel it has to be checked, if the voxel is inside the \ac{CC} or not.
If it is, the current physical properties of this point, \ie{} the orientation of the optical axis and the current layer, from which the absorption and birefringence strength can be derived, has to be saved.
Therefore two a 3d representation exists: a \code{optical\tu axis}, which $(x,y,z)$ value of the optical axis, and a \code{tissue} which stores the integer to the fibers property (see \cref{fig:fiber_discretisation}).
Last can also be used to render a voxel image of the tissue (hence the name).
% 
% 
\par
% 
To be able to calculate that a given point $\vec{p} = (i,j,k)$ is inside the \ac{CC}, the shortest line from the point $\vec{p}$ to the \ac{CC} has to be calculated analog to \cref{sec:collisionDetection}.
Of course in this case this is a little bit more simpler, since one point is already known.
\\
% 
After the shortest distance and point is calculated, it can be used to identify the layer in which it is.
However the conic shape has to be taken into account (see \cref{fig:conical_capsule}.
This can simply be done by interpolating the radii of the \ac{CC} at the found minimal distance point on the \ac{CC}.
From there the physical properties and optical axis is calculated according to the user defined optical model (background, parallel or radial).
\par
% 
But a necessary step is to check, if the current voxel is already occupied.
This happens \eg{} at the border of nerve fiber segments, since there are linked to the next segment.
Therefore a for the user non accessible additional array exists, which stores the last min distance information.
Only if a min distance at this point is calculated, which is smaller then the old one, the information will be overwritten.
\par
% 
% 
...
\\
% 
\itodo{INTEGRATE WITH NEXT PARAGRAPH}
% 
...
\subcaptionbox{\label{fig:vectormodel}vector\tu model with microscopic birefringence myelin layer}[.49\textwidth]{
...
The second uses \mpi{} to allow distribution to multiple \ac{CPU} cores without sharing memory. \todo{openmp in the section, mpi at the end of the chapter}
% 
...
Since the loop runs over all $(i,j,k)$ indices of the three space dimensions, the parallelization can be done using the indices. \todo{a shared cache can be used}
