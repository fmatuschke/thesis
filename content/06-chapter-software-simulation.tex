...
Also the clock frequency is almost at its limits since at that scale the \say{wires} inside the chips become more conductive because they begave as a capacitor.
...
\\[\baselineskip]
% 
The here presented \fastpli{} software package alowes to predict \ac{3D-PLI} experiments within the \python package \pymodule{fastpli.simulation} by means of linear optics \cite{} and 3d fiber models \cite{}.
...
The first part called \name{Generation} \tikzset{external/export next=false} \todo{name} generates a discretizied 3d volume of the nerve fiber models from \cref{chap:modelling}.
