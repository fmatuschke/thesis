...
Since the arrays are quite large, the transfer is performed as an ownership movement of a \name{numpy-array} without having to copy the data.
% 
\begin{lstfloat}[!ht]
\lstset{style=python}
\begin{lstlisting}[]
...
Since the light matter simulation algorithm receives a \name{tissue}, \name{optical\_axis} and \name{property} array, the user can also change the values in between both algorithms.
...
This includes the modalities analysis transmittance, direction and retardation as well as the tilting analysis performed by \ac{ROFL}.
The tilting analysis takes quite a lot of time.
...
Since the sub-volumes are split along a cartesion grid, the cartesion implementations of \ac{MPI} are used (\eg{} \code{MPI\tu Cart\tu create}).
%
\begin{figure}[!t]
    \centering
