...
\end{itemize}}
%
The \ac{3D-PLI} simulation is based on he previous developt tool \textit{simPLI} \cite{Dohmen2015}, which uses linear optics \cite{} to simulate the light tissue inteaction in the \ac{PLI} setup.
...
The vector inside the array at index position $[i,j,k]$ will correspond to the vector at position $p=(i+0.5,j+0.5,k+0.5)\cdot \text{\voxelsize}$}
