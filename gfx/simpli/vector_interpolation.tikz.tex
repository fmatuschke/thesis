...
\begin{tikzpicture}[%
    ...
    \draw[green, thick] (P1) -- (P2) node[below, pos=0.75] {$s$};
        \draw[red, thick] ({0.25*\r*\xa}, {0.25*\r*\ya}) arc(35:35+100/6:0.25*\r) node[right] {$\varphi$};
    % 
    \coordinate (BB1) at (current bounding box.north east);
    \coordinate (BB2) at (current bounding box.south east);
    \path (BB1) -- (BB2) node[right, green, pos=0.42] {linear};
    \path (BB1) -- (BB2) node[right, red, pos=0.58] {spherical};
    \path[] (-1.42*\r,0) -- (1.42*\r,0);
\end{tikzpicture}
\end{tikzsize}